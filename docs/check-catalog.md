...
| `sec-prompt-inject-001` | High / Medium / Low | Caller- or external-system-sourced values spoken in voice prompts without sanitization. HIGH when the prompt is interpreted as **SSML** (Polly parses markup out of the substituted value, so `</speak><speak>` changes what the platform says). MEDIUM for plain text in a whisper/hold/transfer flow, where an **agent** hears caller-authored text as though Connect wrote it. LOW for plain text spoken back to the caller who supplied it — no markup parsing and no second party, so it is reported for traceability, not as an exploitable defect. Cannot determine a value's origin, so it says so and excludes Connect system attributes. |
